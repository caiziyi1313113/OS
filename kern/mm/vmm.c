...
/* do_pgfault - map a page at addr according to vma flags */
int do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr)
{
    ...
    /* stack can grow down: allow addr just below vm_start for VM_STACK */
    if (addr < vma->vm_start)
    {
        if (!(vma->vm_flags & VM_STACK) || addr + PGSIZE < vma->vm_start)
        {
            return -E_INVAL;
        }
    }

    if ((error_code & 1) && !(vma->vm_flags & VM_WRITE))
    {
        return -E_INVAL;
    }
    ...
    uint32_t perm = PTE_U | PTE_V | PTE_A;
    ...
    if (get_page(mm->pgdir, la, NULL) != NULL)
    {
        return 0;
    }
