...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // 分配内核栈，注意除了0号线程，其他每个线程都需要显示分配内核栈
    // setup_kstack调用alloc_page分配8k的内存空间，并修改了pcb中的kstack
    if (setup_kstack(proc) != 0) {
        goto bad_fork_cleanup_proc;
    }

    // call copy_mm to dup OR share mm according clone_flag
    // 复制/共享地址空间（Lab4 仅内核线程：共享/或直接置空）
    // null 
    ...
    // 搭建子进程的 trapframe 与 context
    //区分上下文和中断帧
    //关键一句话：
    //context 保存的是“线程在内核中被挂起时的执行位置”，线程状态切换
    //trapframe 保存的是“进入内核前/线程第一次执行时的现场”，运行状态恢复
    // 该函数，设计trapframe的sp如果为内核其真正执行时的cpu现场就是tr本身，如果是用户态，执行用户栈其为真正线程
    //         设计context的sp为内核栈的tr，第一次用于进程切换后的恢复
...
load_icode(int fd, int argc, char **kargv)
{
    ...
    for (i = 0; i < elf.e_phnum; i++)
    {
        ...
        if ((ret = load_icode_read(fd, &ph, sizeof(struct proghdr),
                                  (off_t)(elf.e_phoff + (uint64_t)i * sizeof(struct proghdr)))) != 0)
        {
        ...
        vm_flags = 0;
        perm = PTE_U | PTE_V | PTE_A;     // 关键：至少加 PTE_A
        if (ph.p_flags & ELF_PF_W) perm |= PTE_D;  // 可选：可写段加 PTE_D（或干脆所有段都加）
        ...
        uintptr_t start = (uintptr_t)ph.p_va, end, la = ROUNDDOWN(start, PGSIZE);
        off_t file_offset = (off_t)ph.p_offset;

        /* copy TEXT/DATA */
        end = (uintptr_t)(ph.p_va + ph.p_filesz);
        while (start < end)
        {
        ...
        /* build BSS */
        end = (uintptr_t)(ph.p_va + ph.p_memsz);
        if (start < la)
        {
            if (start == end)
            {
    ...
    if (pgdir_alloc_page(mm->pgdir, USTACKTOP - PGSIZE, PTE_USER) == NULL ||
        pgdir_alloc_page(mm->pgdir, USTACKTOP - 2 * PGSIZE, PTE_USER) == NULL ||
        pgdir_alloc_page(mm->pgdir, USTACKTOP - 3 * PGSIZE, PTE_USER) == NULL ||
        pgdir_alloc_page(mm->pgdir, USTACKTOP - 4 * PGSIZE, PTE_USER) == NULL)
    {
        ret = -E_NO_MEM;
        goto bad_cleanup_mmap;
    }
    ...
    for (i = argc - 1; i >= 0; i--)
    {
        ...
        if (sp < stacktop - USTACKSIZE)
        {
            ret = -E_TOO_BIG;
            goto bad_cleanup_mmap;
        }

        /* copy string to (new) user stack pages via page2kva */
        uintptr_t dst = sp;
        const char *src = kargv[i];
        size_t left = len;
        while (left != 0)
        {
            uintptr_t la = ROUNDDOWN(dst, PGSIZE);
            struct Page *p = get_page(mm->pgdir, la, NULL);
            if (p == NULL)
            {
                if ((p = pgdir_alloc_page(mm->pgdir, la, PTE_USER)) == NULL)
                {
                    ret = -E_NO_MEM;
                    goto bad_cleanup_mmap;
                }
            ...
            size_t off = dst - la;
            size_t part = PGSIZE - off;
            if (part > left)
            {
                part = left;
            }
            memcpy(page2kva(p) + off, src, part);
            dst += part;
            src += part;
            left -= part;
        }
    ...
    uintptr_t argv_ptrs[EXEC_MAX_ARG_NUM + 1];
    for (i = 0; i < argc; i++)
    {
        argv_ptrs[i] = uargv[i];
    }
    ...
    /* copy argv_ptrs to user stack */
    {
        uintptr_t dst = uargv_ptr;
        const char *src = (const char *)argv_ptrs;
        size_t left = argv_size;
        while (left != 0)
        {
            uintptr_t la = ROUNDDOWN(dst, PGSIZE);
            struct Page *p = get_page(mm->pgdir, la, NULL);
