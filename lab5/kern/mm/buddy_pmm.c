...
static void buddy_check(void) {
    ...
    print_buddy_summary();
    unsigned total_free = nr_buddy_free_pages;
    ...
    assert(total_free == nr_buddy_free_pages);    
    ...
    // === 大块分配后分配小块 ===
    cprintf("\n=== Split Test ===\n");
    unsigned total = nr_buddy_free_pages;
    struct Page *p_big = buddy_alloc_pages(4095);
    assert(p_big != NULL);
    struct Page *p_small1 = buddy_alloc_pages(512);
    struct Page *p_small2 = buddy_alloc_pages(512);
    struct Page *p_small3 = buddy_alloc_pages(3);
    assert(p_small1 && p_small2);

    cprintf("After allocations (split test):\n");
    print_buddy_summary();
    cprintf("free pages now: %u\n", (unsigned)nr_buddy_free_pages);

    buddy_free_pages(p_small1, 512);
    buddy_free_pages(p_small2, 512);
    buddy_free_pages(p_small3, 3);
    buddy_free_pages(p_big, 4096);
    assert(nr_buddy_free_pages == total);
    cprintf("After free (split test) -> free pages: %u\n", (unsigned)nr_buddy_free_pages);
    print_buddy_summary();

    // === 4. 合并测试 ===
    cprintf("\n=== Merge Test ===\n");
    total = nr_buddy_free_pages;
    struct Page *a = buddy_alloc_pages(2);
    struct Page *b = buddy_alloc_pages(2);
    assert(a && b);
    print_page_range(a, 2);
    print_page_range(b, 2);
    cprintf("After allocations (merge test):\n");
    print_buddy_summary();
    buddy_free_pages(a, 2);
    buddy_free_pages(b, 2);
    assert(nr_buddy_free_pages == total);
    cprintf("After merge free :\n");
    print_buddy_summary();
